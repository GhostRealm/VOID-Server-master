...
namespace wServer.logic.loot
{
    ...
    public class Loot : List<ILootDef>
    {
        ...
        private static readonly string[] ForsakenItems = {

        };

        private static readonly string[] VoidItems = {
        ...
        private static void ShowBags(Enemy enemy, IEnumerable<Item> loots, params Player[] owners) {
            ...
            foreach (var i in loots) {
                ...
                if (LegendaryItems.Contains(i.ObjectId))
                    foreach (var p in enemy.Owner.Players.Values)
                        p.SendHelp("<" + owners[0].Name + "> has received a legendary item!: '"
                                                  + i.ObjectId + "'");

                if (ForsakenItems.Contains(i.ObjectId))
                    foreach (var p in enemy.Owner.Players.Values)
                        p.SendHelp("<" + owners[0].Name + "> has received a forsaken item!: '"
                                                  + i.ObjectId + "'");

                if (VoidItems.Contains(i.ObjectId))
                    foreach (var p in enemy.Owner.Players.Values)
                        p.SendHelp("<" + owners[0].Name + "> has obtained a part of the void!: '"
                                                  + i.ObjectId + "'");
